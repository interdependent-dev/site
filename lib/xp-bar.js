...
(function (global) {
  ...
  function gainSound() {
    withCtx(function (ctx, out) {
      tone(ctx, out, { type: 'triangle', freq: 880, dur: 0.07, vol: 0.16 }); // B5
      tone(ctx, out, { type: 'triangle', freq: 1318.5, dur: 0.13, vol: 0.16, t: 0.06 }); // E6
      tone(ctx, out, { type: 'sine', freq: 2637, dur: 0.08, vol: 0.05, t: 0.06 }); // sparkle
    });
  }
  function unlockSound() {
    withCtx(function (ctx, out) {
      tone(ctx, out, { type: 'sine', freq: 150, freqEnd: 55, dur: 0.5, vol: 0.26 }); // boom
      [523.25, 659.25, 783.99, 1046.5].forEach(function (f, i) { // C-E-G-C fanfare
        tone(ctx, out, { type: 'triangle', freq: f, dur: 0.3, vol: 0.15, t: 0.05 + i * 0.09 });
      });
      for (var i = 0; i < 11; i++) { // firework crackle
        tone(ctx, out, { type: 'sine', freq: 1700 + Math.random() * 2000, dur: 0.05, vol: 0.045, t: 0.28 + Math.random() * 0.95 });
      }
    });
  }
  ...
  function levelUp(reward) {
    injectStyles();
    fireworks({ bursts: 12 }); // sky-wide fireworks — the ceremony, not chip confetti
    unlockSound();
    var wrap = document.querySelector('.idxp-toast-wrap');
    if (!wrap) { wrap = el('div', 'idxp-toast-wrap'); document.body.appendChild(wrap); }
