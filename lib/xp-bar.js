...
(function (global) {
  ...
  // One deep firework explosion: low pitched body (boom) + sharp crack + rumble +
  // a little crackle tail. `pitch`/`vol` vary per burst so they don't feel cloned.
  function boom(ctx, out, t, vol, pitch) {
    t = t || 0; vol = vol == null ? 0.55 : vol; pitch = pitch || 1;
    tone(ctx, out, { type: 'sine', freq: 130 * pitch, freqEnd: 38 * pitch, dur: 0.45, vol: 0.55 * vol, t: t });
    tone(ctx, out, { type: 'triangle', freq: 90 * pitch, freqEnd: 30 * pitch, dur: 0.55, vol: 0.32 * vol, t: t });
    noiseHit(ctx, out, { t: t, dur: 0.1, vol: 0.45 * vol, type: 'highpass', freq: 2000, freqEnd: 700 }); // crack
    noiseHit(ctx, out, { t: t, dur: 0.5, vol: 0.34 * vol, type: 'lowpass', freq: 600, freqEnd: 110 }); // boom rumble
    for (var i = 0; i < 4; i++) { // crackle tail
      noiseHit(ctx, out, { t: t + 0.09 + Math.random() * 0.45, dur: 0.05, vol: 0.1 * vol, type: 'bandpass', freq: 1200 + Math.random() * 2600, q: 7 });
    }
  }
  function withCtx(fn) {
  ...
  // A cascade of stacked twinkle pings — a sparkle SHOWER to match the confetti,
  // not a single note. Two quick lead notes + a scatter of high shimmer.
  function gainSound() {
    withCtx(function (ctx, out) {
      tone(ctx, out, { type: 'triangle', freq: 880, dur: 0.07, vol: 0.15 }); // B5 lead
      tone(ctx, out, { type: 'triangle', freq: 1318.5, dur: 0.12, vol: 0.15, t: 0.055 }); // E6 lead
      var shimmer = [1568, 1760, 2093, 2637, 3136, 2349, 1976]; // G6 A6 C7 E7 G7 D7 B6
      for (var i = 0; i < 7; i++) {
        var f = shimmer[(Math.random() * shimmer.length) | 0];
        tone(ctx, out, { type: 'sine', freq: f, dur: 0.06 + Math.random() * 0.05, vol: 0.04 + Math.random() * 0.035, t: 0.03 + i * 0.035 + Math.random() * 0.02 });
      }
    });
  }
  ...
  function fireworks(opts) {
    ...
    if (actx) {
      ...
      aout.gain.value = 0.9;
      aout.connect(actx.destination);
      tone(actx, aout, { type: 'sine', freq: 300, freqEnd: 1100, dur: 0.22, vol: 0.06 }); // launch whoosh
    }
  ...
  function toast(amount, label) {
    ...
    wrap.appendChild(t);
    // Dopamine: confetti burst from the toast, intensity scaled by the gain.
    var n = parseInt(String(amount).replace(/[^0-9]/g, ''), 10) || 0;
    confetti({ count: Math.round(70 + n * 0.9) });
    setTimeout(function () {
  ...
  function sync(handle, base) {
    return fetchXp(handle, base).then(function (xp) {
      if (!xp) return null;
      var now = unlockedPerkKeys(xp);
      var prev = _seen[handle];
      if (prev) {
        var fresh = Object.keys(now).filter(function (k) { return !prev[k]; });
        if (fresh.length) {
